...
namespace Figletize.Generator;

[Generator]
public sealed class FigletizeSourceGenerator : ISourceGenerator
{
    ...
    public static readonly DiagnosticDescriptor NestedTypeIsNotSupportedDiagnostic = new(
        "FGL0005",
        "Figletize generation does not support nested types",
        "Unable to generate Figletize text for nested type '{0}'. Generation is only supported for non-nested types.",
        category: "Figletize",
        defaultSeverity: DiagnosticSeverity.Error,
        isEnabledByDefault: true);
    ...
    public const string AttributeSource =
        ...
        namespace Figletize
        {
            ...
            internal sealed class GenerateFigletizeTextAttribute : Attribute
            {
                public string MemberName { get; }
                public string FontName { get; }
                public string SourceText { get; }

                public GenerateFigletizeTextAttribute(string memberName, string fontName, string sourceText)
                {
    ...
    {
        context.AddSource("GenerateFigletizeTextAttribute.cs", AttributeSource);
        ...
        if (context.SyntaxContextReceiver is Receiver receiver)
        {
            ...
            foreach (var pair in receiver.DataByType)
            {
                ...
                {
                    var font = FigletizeFonts.TryGetByName(item.FontName);
                ...
                var hintName = type.Namespace is null
                    ? $"{type.Class}.FigletizeGenerated.cs"
                    : $"{type.Namespace}.{type.Class}.FigletizeGenerated.cs";
                ...
                foreach (var attributeList in classDeclaration.AttributeLists)
                {
                    foreach (var attribute in attributeList.Attributes)
                    {
                        if (attribute.Name is IdentifierNameSyntax name &&
                            (name.Identifier.Text == "GenerateFigletizeTextAttribute" || name.Identifier.Text == "GenerateFigletizeText") &&
